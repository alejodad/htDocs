...
namespace LibLlenarGrid
{
    public class ClsLlenarGrid
    {
        ...
        public bool LlenarGrid(DataGridView grdGenerico)
        {
            ...
            if (strNombreTabla == "") {
                strNombreTabla = "LlenarGrid";// se le da un nombre 
            }
            if (objConexionBD.LlenarDataSet(strNombreTabla, strSQL, false))
            {
                grdGenerico.DataSource = objConexionBD.DataSet_Retornado.Tables[strNombreTabla];//retorna lo que hay en el resultado del procedure
